// Thống kê người chơi CỜ ÚP: Elo, số ván thắng/thua/hòa, độ chính xác trung bình.
...
export type JqGameResultForStats = 'win' | 'loss' | 'draw'

const STORAGE_KEY = 'chessone.jieqi.userstats.v1'
...
const DEFAULT_STATS: JqUserStats = {
  elo: 1200,
  totalGames: 0,
  wins: 0,
  losses: 0,
...
type StatsListener = (stats: JqUserStats) => void
...
function loadStats(): JqUserStats {
  ...
  try {
    ...
    console.warn('[jq-stats] lỗi đọc storage:', e)

    return { ...DEFAULT_STATS }
  }
}

function saveStats(stats: JqUserStats): boolean {
  ...
  try {
    ...
    console.warn('[jq-stats] lỗi ghi storage:', e)
...
export function getStats(): JqUserStats {
  return loadStats()
}

// Thống kê người chơi: Elo, số ván thắng/thua/hòa, độ chính xác trung bình.
// Lưu bằng localStorage nên CHỈ dùng được ở phía trình duyệt (client component/hook),
// KHÔNG dùng được trong Server Component hay Route Handler.
...
export type GameResultForStats = 'win' | 'loss' | 'draw'

const STORAGE_KEY = 'chessone.chess.userstats.v1'

/** Elo "giả định" ứng với từng cấp độ máy (1 = yếu nhất, 10 = mạnh nhất) - dùng để tính Elo sau mỗi ván */
...
/** Tên gọi Hán-Việt cho từng cấp độ máy, dùng để hiển thị trong bộ chọn cấp độ */
...
return { ...DEFAULT_STATS, ...data }
    ...
    console.warn('[stats] lỗi đọc storage:', e)
    
return { ...DEFAULT_STATS }
  }
}

function saveStats(stats: UserStats): boolean {
...
return true
    ...
    console.warn('[stats] lỗi ghi storage:', e)
...
/** Ước lượng độ chính xác 1 nước đi (0-100) từ mức tổn thất điểm centipawn so với nước tốt nhất (dùng nếu sau này có tính năng phân tích bằng engine) */
...
/** Công thức Elo chuẩn (hệ số K=32), có chặn dưới ELO_MIN để tránh Elo âm hoặc quá thấp */
...
/** Cập nhật Elo + số ván thắng/thua/hòa sau khi 1 ván đấu với máy kết thúc. Trả về Elo cũ/mới để hiển thị "+15" chẳng hạn. */
export function addGameResult(
  level: number,
  result: GameResultForStats
...
/** Ghi nhận độ chính xác của 1 nước đi (nếu app có tính năng phân tích bằng engine ở tương lai) */
...
/** Đăng ký callback được gọi mỗi khi stats thay đổi (dùng để đồng bộ UI hiển thị Elo ở nơi khác nếu cần) */

// Lưu & đọc lịch sử ván CỜ CARO (localStorage), và chia sẻ ván qua đường link.
...
export interface GmStoredGame {
  ...
  /** Màu quân người chơi cầm: Đen đi trước */
  userColor: 'black' | 'white'

  /** Luật thắng của ván này */
  rule?: string
  vsEngine: boolean
  level?: number
...
const STORAGE_KEY = 'chessone.gomoku.history.v1'
...
function loadAll(): GmStoredGame[] {
  ...
  try {
    ...
    console.warn('[gm-history] lỗi đọc storage:', e)

    return []
  }
}

function saveAll(games: GmStoredGame[]): boolean {
  ...
  try {
    ...
    console.warn('[gm-history] lỗi ghi storage (có thể vượt quota):', e)
...
export function saveGame(game: GmStoredGame): boolean {
  if (!game || !game.id) {
    console.error('[gm-history] game thiếu id')
...
export function listGames(): GmStoredGame[] {
  return loadAll()
}

export function getGame(id: string): GmStoredGame | null {
...
interface SharePayload {
  v: number
  k: string
  g: Omit<GmStoredGame, 'id'>
}

function buildSharePayloadGame(game: GmStoredGame): Omit<GmStoredGame, 'id'> {
...
export function encodeGame(game: GmStoredGame): string {
...
export function decodeGame(token: string): Omit<GmStoredGame, 'id'> | null {
  try {
    ...
    console.warn('[gm-share] không giải mã được link:', e)
...
export function buildShareUrl(game: GmStoredGame): string {
...
export function parseShareFromUrl(): Omit<GmStoredGame, 'id'> | null {
 ...
 */
export function importShared(game: Omit<GmStoredGame, 'id'> | null): GmStoredGame | null {
  if (!game) return null
  let g: GmStoredGame

  try {
    g = { ...(JSON.parse(JSON.stringify(game)) as Omit<GmStoredGame, 'id'>), id: '' }
  } catch {
    return null
  }

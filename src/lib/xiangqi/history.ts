// Lưu & đọc lịch sử ván CỜ TƯỚNG (localStorage), và chia sẻ ván qua đường link.
// Cấu trúc giống lib/chess/history.ts nhưng dùng khóa lưu trữ riêng nên lịch sử
// cờ vua và cờ tướng không lẫn vào nhau. CHỈ dùng được ở phía trình duyệt.

export interface XqStoredGame {
  id: string
  gameKey: 'xiangqi'
  ...
  /** Chỉ số thế cờ chấp (0 = ván thường) - xem STARTUP_FEN trong lib/xiangqi/constants.ts */
  handicap?: number
  startedAt: number
  endedAt?: number
  moveCount: number
...
function loadAll(): XqStoredGame[] {
  ...
  try {
    ...
    console.warn('[xq-history] lỗi đọc storage:', e)

    return []
  }
}

function saveAll(games: XqStoredGame[]): boolean {
  ...
  try {
    ...
    console.warn('[xq-history] lỗi ghi storage (có thể vượt quota):', e)
...
export function saveGame(game: XqStoredGame): boolean {
  if (!game || !game.id) {
    console.error('[xq-history] game thiếu id')
...
export function listGames(): XqStoredGame[] {
  return loadAll()
}

export function getGame(id: string): XqStoredGame | null {
...
interface SharePayload {
  v: number
  k: string
  g: Omit<XqStoredGame, 'id'>
}

function buildSharePayloadGame(game: XqStoredGame): Omit<XqStoredGame, 'id'> {
...
export function encodeGame(game: XqStoredGame): string {
...
export function decodeGame(token: string): Omit<XqStoredGame, 'id'> | null {
  try {
    ...
    console.warn('[xq-share] không giải mã được link:', e)
...
export function buildShareUrl(game: XqStoredGame): string {
...
export function parseShareFromUrl(): Omit<XqStoredGame, 'id'> | null {
 ...
 */
export function importShared(game: Omit<XqStoredGame, 'id'> | null): XqStoredGame | null {
  if (!game) return null
  let g: XqStoredGame

  try {
    g = { ...(JSON.parse(JSON.stringify(game)) as Omit<XqStoredGame, 'id'>), id: '' }
  } catch {
    return null
  }

// Thống kê người chơi CỜ TƯỚNG: Elo, số ván thắng/thua/hòa, độ chính xác trung bình.
// Giống lib/chess/user-stats.ts nhưng dùng khóa lưu trữ riêng để Elo cờ vua và
// cờ tướng độc lập với nhau. CHỈ dùng được ở phía trình duyệt.

export interface XqUserStats {
  elo: number
  totalGames: number
  wins: number
  losses: number
...
export type XqGameResultForStats = 'win' | 'loss' | 'draw'
...
const DEFAULT_STATS: XqUserStats = {
  elo: 1200,
  totalGames: 0,
  wins: 0,
  losses: 0,
...
type StatsListener = (stats: XqUserStats) => void
...
function loadStats(): XqUserStats {
  ...
  try {
    ...
    console.warn('[xq-stats] lỗi đọc storage:', e)

    return { ...DEFAULT_STATS }
  }
}

function saveStats(stats: XqUserStats): boolean {
  ...
  try {
    ...
    console.warn('[xq-stats] lỗi ghi storage:', e)
...
export function getStats(): XqUserStats {
  return loadStats()
}
